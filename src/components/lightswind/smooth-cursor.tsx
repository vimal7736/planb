...
export function SmoothCursor({
  ...
  // Custom dynamic pointer for hover state
  const hoverCursor = (
    <div 
      className="w-8 h-8 rounded-full flex items-center justify-center border-2 border-[#556B2F] bg-[#556B2F]/20 backdrop-blur-md"
      style={{ boxShadow: "0 0 15px rgba(85, 107, 47, 0.4)" }}
    >
      <div className="w-1.5 h-1.5 rounded-full bg-[#556B2F]" />
    </div>
  );

  const cursorElement = isHovering ? hoverCursor : (cursor || defaultCursor);
  ...
  useEffect(() => {
    ...
    const smoothMouseMove = (e: MouseEvent) => {
      ...
      // We check if it's a known clickable tag OR if it has a pointer cursor in CSS (which catches onClick divs)
      if (
        target.closest("a") || 
        ...
        // Walk up the tree slightly to check computed styles for cursor: pointer
